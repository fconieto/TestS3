 ...
 * <P>
 * <A
 * HREF="/resources/classes/Acme/JPM/Encoders/GifEncoder.java">Fetch the
 * software. </A> <BR>
...
public class GifEncoder extends ImageEncoder {
    ...
    /**
     * Original encodeDone method. Calls new encodeDone method with a quanitized
     * palette.
     */
    ...
    /**
     * Modified encodeDone that takes a palette of reduced colors. If the
     * palette exists, it expects that the internal rgbPixels[][] contains
     * indexes into the palette. If the palette is null, then it assumes that
     * the rgbPixels[][] contains colors. If more than 255 colors are needed
     * (which should only happen when the palette is null), this method gets
     * called again internally after the rgbPixels are quantized.
     */
            ...
            for (int col = 0; col < width; ++col) {
                ...
                if (item == null) {
                    if (index >= 256) {
                        ...
                        encodeDone(Quantize.quantizeImage(rgbPixels, 255));
                        return;
